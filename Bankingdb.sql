...
select c.customerName , e.firstname from customers c
left join employees e
on c.salesRepEmployeeNumber = e.employeeNumber;

-- 2. Calculate the total number of orders placed by each customer. Show the customer name and the order count.
select c.customerNumber , c.customerName, count(o.customerNumber) as OrderCount from  customers c
left join orders o
on c.customerNumber = o.customerNumber
group by c.customerNumber;

-- 3. Find the total payment amount received from each customer. Show the customer name and the total amount.
select c.customerNumber , c.customerName , sum(p.amount) as Total_Amount from customers c
left join payments p
on c.customerNumber =p.customerNumber
group by c.customerNumber;

-- 4. List all products (product name, product line) that have never been ordered. (Hint: Use  a LEFT JOIN and check for NULL).
select productname , productline , od.orderNumber from products p
left join orderdetails od
on p.productCode = od.productCode
where od.orderNumber is null;

-- 5. Show the product name and the total quantity ordered for each product, but only for products where the total quantity ordered is greater than 1000.
select p.productname , sum(od.quantityOrdered) as Total_OD_ForEach from products p
join orderdetails od 
on p.productCode = od.productCode
group by p.productName
having Total_OD_ForEach > 1000;
...
select oc.city , count(e.officeCode) as TotalEmp from offices oc
join employees e 
...
-- 5. Get the list of employees who report to someone else (i.e., they have a reportsTo value). Show their name and their manager's name.
select concat_ws(" ",emp.firstname ,emp.lastname ) as Employee_Name , concat_ws(" ",mgr.Firstname ,mgr.lastname)
from employees emp
left join employees mgr
on emp.reportsTo = mgr.employeeNumber;

-- Question Set 4:
-- 1. For each order, list the order number, customer name, and the name of the sales rep
-- responsible for that customer.
select o.orderNumber ,c.customername  , concat_ws(" " , e.firstname,e.lastname) as NamesalesRep from orders o
left join customers c
on c.customerNumber = o.customerNumber
join employees e
on c.salesRepEmployeeNumber = e.employeeNumber;
...
-- 3. Show the product line and the average buy price for products within that product line, but
-- only show product lines whose average buy price is above $50.
select productLine, avg(buyPrice) as AVG_BUY from products
group by productLine
having AVG_BUY > 50;



-- 4. List all products that were ordered in 2005. Show the product code, product name, and the
-- total quantity ordered in that year.
select p.productcode , p.productname , sum(od.quantityOrdered), year(o.orderdate) as Year_ from products p 
...
-- Question Set 5:
-- 1. Calculate the total value of inventory for each product line. (Value = quantityInStock *  buyPrice).
select productline , sum(quantityinstock * buyprice )as total_value_Inventory from products
group by productLine ;

-- 2. Identify orders where the total value of the order (sum of quantityOrdered * priceEach for all
-- items in the order) is greater than $50,000.
select ordernumber , sum(quantityOrdered *priceEach ) from orderdetails
group by orderNumber;


-- 3. Find the month and year with the highest total sales amount (from payments). Format the
-- result like "2004-08".
select concat(year(paymentDate),"-",month(paymentdate)) as M_Y , sum(amount) from payments 
group by M_Y 
order by sum(amount) desc
limit 1;


-- 4. List the employees who work in the same office as their manager. (This requires a self-join on
-- the employees table and a join to the offices table twice).
select e.firstname as EmployeeName , mgr.firstname as MnagerName , e.officeCode  from employees e
join employees mgr
on e.reportsTo = mgr.employeeNumber
where  e.officeCode = mgr.officeCode;


-- 5. For each customer, find the difference between their credit limit and the total amount they
-- have spent (from payments). Show only customers where this difference is less than
-- $10,000
SELECT c.customerName,
       c.creditLimit,
       SUM(p.amount) AS totalSpent,
       c.creditLimit - SUM(p.amount) AS difference
FROM customers c
JOIN payments p
ON c.customerNumber = p.customerNumber
GROUP BY c.customerNumber, c.customerName, c.creditLimit
HAVING c.creditLimit - SUM(p.amount) < 10000;
